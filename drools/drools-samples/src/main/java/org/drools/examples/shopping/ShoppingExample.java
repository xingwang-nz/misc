...
public class ShoppingExample {
    ...
    public static final void main(String[] args) {
        ...
        final KieSession ksession = kc.newKieSession("ShoppingKS");

        final Customer mark = new Customer("mark", 0);
        ksession.insert(mark);

        final Product shoes = new Product("shoes", 60);
        ksession.insert(shoes);

        final Product hat = new Product("hat", 60);
        ksession.insert(hat);

        ksession.insert(new Purchase(mark, shoes));

        final FactHandle hatPurchaseHandle = ksession.insert(new Purchase(mark, hat));
